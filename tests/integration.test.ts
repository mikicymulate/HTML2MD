...
const fixture = join(here, 'fixtures', 'article.html');
...
describe('extractPage (integration, real Chromium)', () => {
  it('produces clean AI Markdown with an element map from a local HTML file', async () => {
    ...
    const hero = res.images.find((i) => i.description === 'A red widget on a workbench');
    expect(hero?.kept).toBe(true);
    const pixel = res.images.find((i) => i.src.includes('pixel.gif'));
